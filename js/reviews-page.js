...
(function () {
  ...
  const certRoot = document.querySelector("[data-cert-groups]");
  if (certRoot) {
    const filterBtns = certRoot.querySelectorAll("[data-cert-filter]");
    const sections = certRoot.querySelectorAll("[data-cert-category]");

    function setCertFilter(filter) {
      filterBtns.forEach((btn) => {
        const on = btn.getAttribute("data-cert-filter") === filter;
        btn.classList.toggle("is-active", on);
        btn.setAttribute("aria-pressed", on ? "true" : "false");
      });
      sections.forEach((section) => {
        const show = filter === "all" || section.getAttribute("data-cert-category") === filter;
        section.hidden = !show;
      });
    }
    ...
    filterBtns.forEach((btn) => {
      btn.addEventListener("click", () => {
        setCertFilter(btn.getAttribute("data-cert-filter") || "all");
      });
    });
